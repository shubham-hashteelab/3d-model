...
class ProcessFilesRequest(BaseModel):
    """Request model for file processing API."""

    num_frames: int = 10  # Number of frames to extract from video (default 10)
...
<body>
    ...
    async def process_files_with_camera(
        ...
        print(f"[process-files-with-camera] Received {len(files)} files, output_format={output_format}")
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                for file in files:
                    ...
                    if file.filename.lower().endswith(('.mp4', '.avi', '.mov', '.mkv', '.flv', '.wmv', '.webm', '.m4v')):
                        # Video file - extract frames
                        frames_dir = os.path.join(temp_dir, f"frames_{file.filename}")
                        os.makedirs(frames_dir, exist_ok=True)
                        
                        # Calculate FPS to get approximately num_frames frames
                        cap = cv2.VideoCapture(file_path)
                        if cap.isOpened():
                            total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
                            video_fps = cap.get(cv2.CAP_PROP_FPS)
                            duration = total_frames / video_fps if video_fps > 0 else 10.0  # Default 10s if can't read
                            cap.release()
                            
                            # Calculate target FPS to get approximately num_frames
                            target_fps = max(1.0, num_frames / duration)
                        else:
                            target_fps = 10.0  # Default fallback
                        
                        frame_paths = VideoHandler.process(file_path, frames_dir, fps=target_fps)
                        processed_images.extend(frame_paths)
                        
                    elif file.filename.lower().endswith(('.png', '.jpg', '.jpeg', '.webp', '.bmp', '.tiff', '.tif')):
                ...
                # Run inference with default gradio settings
                prediction = model.inference(
                    image=processed_images,
                    export_dir=None,  # Don't save to disk
                    export_format="glb",
