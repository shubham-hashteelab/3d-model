...
class EventHandlers:
    ...
    def gradio_demo(
        ...
        # If target_dir is None or "None", try to find the latest uploaded session
        if target_dir is None or target_dir == "None":
            ...
            if os.path.exists(input_images_dir):
                sessions = [d for d in os.listdir(input_images_dir) if d.startswith("session_")]
                if sessions:
                    # Sort by timestamp (session_YYYYMMDD_HHMMSS_ffffff)
                    latest_session = max(sessions, key=lambda x: x.split("_")[1] + "_" + x.split("_")[2])
                    target_dir = os.path.join(input_images_dir, latest_session)
                    print(f"Recovered target_dir from latest session: {target_dir}")
        ...
        try:
            print(f"[DEBUG] Starting reconstruction process for {target_dir}")
            with torch.no_grad():
